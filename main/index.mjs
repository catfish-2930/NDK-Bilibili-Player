      ...
      (response) => {
        ...
        response.on('end', () => {
          if (response.statusCode !== 200)
            return reject(new Error(`Bilibili search failed (${response.statusCode}).`))
          try {
            resolve(JSON.parse(body))
          } catch {
            reject(new Error('Bilibili returned invalid JSON.'))
          }
        })
      }
    )
...
function getMixinKey(imgUrl, subUrl) {
  ...
  return MIXIN_KEY_ENC_TAB.map((index) => source[index])
    .join('')
    .slice(0, 32)
}
...
async function search(query, page) {
  ...
  const videoGroup = (response.data?.result || []).find((group) => group.result_type === 'video')
  const pageInfo = response.data?.pageinfo?.video || response.data?.page || {}
  const rawTotalPages = Number(
    pageInfo.numPages ||
      pageInfo.num_pages ||
      response.data?.numPages ||
...
export function register({ ipcMain, plugin, getConfig, channelPrefix }) {
  ...
  registered = true
  for (const action of ['recommend', 'search', 'pages', 'queue'])
    ipcMain.removeHandler(`${channelPrefix}:${action}`)
  ...
  ipcMain.handle(`${channelPrefix}:pages`, async (_event, video) => {
    try {
      return { ok: true, pages: await getVideoPages(video) }
    } catch (error) {
      return { ok: false, error: error.message }
    }
  })
  ipcMain.handle(`${channelPrefix}:queue`, async (_event, video) => {
    ...
    if (!id || !/^https:\/\/www\.bilibili\.com\/video\/BV/i.test(webpageUrl))
      return { ok: false, error: 'Invalid Bilibili video.' }
    const selectedUrl =
      page > 1 ? `${webpageUrl}${webpageUrl.includes('?') ? '&' : '?'}p=${page}` : webpageUrl
    return {
      ok: true,
      mediaItem: {
        title: String(video.title || id),
        artist: String(video.artist || ''),
        path: selectedUrl,
        thumbnail: String(video.thumbnail || ''),
        sourcePluginId: plugin.id,
        externalId: id,
        mediaSource: {
          backend: 'libmpv-bilibili',
          webpageUrl: selectedUrl,
          title: String(video.title || id),
          thumbnail: String(video.thumbnail || '')
        }
      }
