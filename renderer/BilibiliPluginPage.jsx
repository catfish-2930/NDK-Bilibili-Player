...
function BilibiliIcon() {
  return <svg className="bilibili-brand-icon" viewBox="0 0 36 32" aria-hidden="true"><path d="M10 7 7 3M26 7l3-4M7 8h22a4 4 0 0 1 4 4v11a6 6 0 0 1-6 6H9a6 6 0 0 1-6-6V12a4 4 0 0 1 4-4Z" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/><path d="M12 17h.01M24 17h.01M12 23c3.5 2 8.5 2 12 0" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round"/></svg>
}
...
function BilibiliPluginPage({ onEnqueueMedia, onShowToast, KeyboardComponent }) {
  ...
  const load = async (nextQuery, nextPage = 1) => {
    ...
    try {
      ...
      setPage(Number(result.page || nextPage)); setTotalPages(result.totalPages ? Math.max(1, Number(result.totalPages)) : null); setHasNext(Boolean(result.hasNext))
  ...
  const loadRecommendation = async () => {
    setLoading(true); setMessage('')
    try {
      const result = await window.api.plugins.invoke('plugin:bilibili-player:recommend', { page: 1 })
    ...
    } catch (error) { setVideos([]); setMessage(error.message || 'Bilibili recommendation failed.') } finally { setLoading(false) }
  }
  ...
  const selectPart = async (part) => {
    ...
    if (!picker) return
    setPartPicker(null)
    await enqueue({ ...picker.video, page: part.page, title: `${picker.video.title} - ${part.title}` })
  }
  ...
  return <section className="bilibili-plugin-page">
    ...
    <div className="bilibili-footer"><div className="bilibili-search-wrap"><input value={query} onChange={(e) => setQuery(e.target.value)} onFocus={() => setKeyboardOpen(true)} onClick={() => setKeyboardOpen(true)} onKeyDown={(e) => e.key === 'Enter' && load(query)} inputMode="none" placeholder="请输入关键词搜索 Bilibili" /><button type="button" onClick={() => load(query)}><Search size={28} /></button></div>
      <div className="bilibili-pagination"><button type="button" disabled={loading || page <= 1} onClick={() => load(query, page - 1)}><ChevronLeft />上一页</button><span>{totalPages ? `${page} / ${totalPages}` : page}</span><button type="button" disabled={loading || !hasNext} onClick={() => load(query, page + 1)}>下一页<ChevronRight /></button></div></div>
    {partPicker && <div className="bilibili-part-modal" role="dialog" aria-modal="true" aria-label="选择视频分片"><div className="bilibili-part-dialog"><div className="bilibili-part-heading"><strong>选择分片</strong><button type="button" onClick={() => setPartPicker(null)}>关闭</button></div><div className="bilibili-part-list">{partPicker.pages.map((part) => <button type="button" key={part.cid} onClick={() => selectPart(part)}><b>P{part.page}</b><span>{part.title}</span>{part.duration > 0 && <small>{Math.floor(part.duration / 60)}:{String(part.duration % 60).padStart(2, '0')}</small>}</button>)}</div></div></div>}
